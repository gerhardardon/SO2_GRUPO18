...
int main(){
    ...
    // añadimos la opcion para que el usuario ingrese la ruta del archivo
    char filename[100];
    ...
    write_file("my_json.json", new_json);   //  escribimos el json en un archivo
}*/

// Compilar: gcc read_json.c cJSON.c -o read_json
// Ejecutar: ./read_jsonS
...
void print_json_object(char *json_string) { //  creamos una funcion para imprimir el json
    cJSON *json = cJSON_Parse(json_string); //  parseamos el json
    char *formatted_json = cJSON_Print(json);   //  formateamos el json
    printf("%s\n", formatted_json); //  imprimimos el json
    cJSON_Delete(json); //  eliminamos el json
    free(formatted_json);   //  liberamos la memoria
}
...
void read_json_file(char* filename){ //  creamos una funcion para leer el archivo json
    ...
    int n = 0; //  inicializamos un contador
    cJSON *item = NULL; //  creamos un objeto json
    cJSON_ArrayForEach(item, json){ //  iteramos sobre el json
        if(cJSON_IsObject(item)){ //  verificamos que sea un objeto
            cJSON *no_cuenta = cJSON_GetObjectItem(item, "no_cuenta"); //  obtenemos el valor del objeto
            cJSON *nombre_ = cJSON_GetObjectItem(item, "nombre"); 
            cJSON *saldo_ = cJSON_GetObjectItem(item, "saldo"); 
            ...
            if(cJSON_IsNumber(no_cuenta) && cJSON_IsString(nombre_) 
                && cJSON_IsNumber(saldo_))//  verificamos que los valores sean numericos y string
                {
    ...
    usuarios_size = n; //  guardamos el tamaño de los usuarios
...
char* struct_to_json()  //  convertimos la estructura a json
{

    
    cJSON *json_array = cJSON_CreateArray(); //  creamos un arreglo json
...
void write_file(char* filename, char* data) //  escribimos el json en un archivo
{
    //  abrimos el archivo
